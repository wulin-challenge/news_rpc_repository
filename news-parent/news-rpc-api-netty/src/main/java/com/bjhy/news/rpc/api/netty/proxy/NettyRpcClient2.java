 ...
 * @author wubo
 */
public class NettyRpcClient2 extends AbstractNotifyListener{
	
    private static final Logger LOGGER = LoggerFactory.getLogger(NettyRpcClient2.class);
    ...
    public NettyRpcClientHandler connectServer(RpcRequest request) {
    	...
    	if(!NettyRpcClient2.cacheConnect.containsKey(address)){
    		connectServer(request.getHost(), request.getPort());
    	}
    	return NettyRpcClient2.cacheConnect.get(address);
    }
	...
	public static NettyRpcClient2 getInstance(){
		...
		NettyRpcClient2 nettyRpcClient = (NettyRpcClient2) notifyListener;
		if(nettyRpcClient == null){
			nettyRpcClient = new NettyRpcClient2();
		}
